...
export class Validator {
  ...
  public validateOutput(filesToValidate: Record<string, string>, allFiles: Record<string, string>, dependencyGraph: DependencyNode[], prompt: string = ""): string[] {
    ...
    errors.push(...this.validateJSXExtension(filesToValidate));
    errors.push(...this.validateNoJSFiles(filesToValidate));
    return errors;
  }
  ...
  public validateRouterWrapping(filesToValidate: Record<string, string>, allFiles: Record<string, string>): string[] {
    ...
    if (usesRouter) {
      ...
      if (!hasRouterWrapper) {
        errors.push(`🚨 CRITICAL ERROR: You are using "react-router-dom" (e.g., useNavigate, <Link>), but your app is NOT wrapped in a router. You MUST wrap your <App /> component inside a <MemoryRouter> or <HashRouter> in "src/main.tsx" or "src/App.tsx" to prevent routing crashes in the preview iframe. Do NOT use BrowserRouter.`);
      }
    }
        ...
        if (imp.startsWith('.') || imp.startsWith('@/')) {
          ...
          if (!resolved) {
            ...
            if (caseInsensitiveMatch) {
              ...
              errors.push(`🚨 CRITICAL ERROR: Missing import target "${imp}" in file "${path}". You referenced this file but it does not exist in the project. You MUST create this missing file or fix the import path. If this is a React project, ensure "src/App.tsx" and "src/index.css" are present.`);
            }
          }
        }
      }
    ...
    for (const fileName of Object.keys(files)) {
      ...
      if (isSourceFile && !isConfigFile && !isRootEntry) {
        if (!fileName.startsWith('src/')) {
          errors.push(`🚨 CRITICAL ERROR: File "${fileName}" must be placed inside the "src/" directory. Root-level clutter causes path confusion.`);
        }
      }
    }
    return errors;
  ...
  public validateNoJSFiles(files: Record<string, string>): string[] {
    const errors: string[] = [];
    for (const fileName of Object.keys(files)) {
      if (fileName.endsWith('.js') && !['tailwind.config.js', 'postcss.config.js'].includes(fileName)) {
        errors.push(`🚨 CRITICAL ERROR: JavaScript file "${fileName}" is not allowed. You MUST use TypeScript (.ts or .tsx).`);
      }
  ...
  public validateForbiddenPatterns(files: Record<string, string>): string[] {
    ...
    for (const [fileName, content] of Object.entries(files)) {
      ...
      // 4. Detect unhandled Supabase environment variables (DISABLED to prevent blocking AI)
      /*
