...
interface MessageItemProps {
  ...
  phase?: BuilderPhase;
  saveProjectConfig?: (config: any) => void;
}

const MessageItem: React.FC<MessageItemProps> = ({ message: m, index: idx, handleSend, isLatest, phase, saveProjectConfig }) => {
  const { t } = useLanguage();
  const [copiedSql, setCopiedSql] = useState(false);
  const [selectionMade, setSelectionMade] = useState(false);

  const sqlFile = m.files && (m.files['database.sql'] || m.files['supabase/schema.sql']);
  
  const isLocal = m.role === 'assistant' && !m.model?.includes('/') && (
          ...
          {(m.content || m.image || (m.plan && m.plan.length > 0) || (m.files && Object.keys(m.files).length > 0) || (m.isApproval && isLatest && !selectionMade) || (m.questions && m.questions.length > 0)) && (
            ...
            `} style={{ wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
              ...
              {m.plan && m.plan.length > 0 && m.role === 'assistant' && (
                <PlanBlock plan={m.plan} isLocal={isLocal} activePlanIndex={m.activePlanIndex} completedPlanIndices={m.completedPlanIndices} />
              )}
              ...
              {m.questions && m.questions.length > 0 && !m.answersSummary && (
                <Questionnaire 
                  ...
                  onComplete={(answers, rawAnswers) => {
                    // Check if there are supabase credentials
                    const supabaseQ = m.questions.find((q: any) => q.type === 'supabase_credentials');
                    if (supabaseQ && rawAnswers && rawAnswers[supabaseQ.id]) {
                      const creds = rawAnswers[supabaseQ.id];
                      if (creds.url && creds.key && saveProjectConfig) {
                        saveProjectConfig({
