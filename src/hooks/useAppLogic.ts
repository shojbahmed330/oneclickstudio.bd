...
import { User as UserType, Project, BuilderPhase, ProjectConfig } from '../types';
...
export const useAppLogic = (user: UserType | null, setUser: (u: UserType | null) => void) => {
  ...
  return {
    ...
    saveProjectConfig: async (config: Partial<ProjectConfig>) => {
      const newConfig = { ...projectManager.projectConfig, ...config };
