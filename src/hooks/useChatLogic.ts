...
export const useChatLogic = (
  ...
  const isSupabaseIntent = useCallback((text: string): boolean => {
    const normalized = (text || '').toLowerCase();
    return /(supabase|database|backend|auth|login|signup|store|save|fetch|api|admin|dashboard)/.test(normalized);
  }, []);

  const shouldAllowSupabaseQuestion = useCallback((promptText: string, messagesSnapshot: ChatMessage[], currentProjectConfig: ProjectConfig): boolean => {
    // If already configured, don't ask again
    if (currentProjectConfig?.supabase_url && currentProjectConfig?.supabase_key) {
      return false;
    }

    // If the prompt itself is the answer to the credentials question, don't ask again
    if (promptText.includes('URL:') && promptText.includes('Key:') && promptText.includes('VITE_SUPABASE_URL')) {
      return false;
    }

    if (isSupabaseIntent(promptText)) return true;

    // Also allow during follow-up turns if user previously asked for something requiring supabase
    for (let i = messagesSnapshot.length - 1; i >= 0; i--) {
      const msg = messagesSnapshot[i];
      if (msg.role !== 'user') continue;
      if (isSupabaseIntent(msg.content || '')) return true;
    }
...
INSTRUCTION: Analyze the test failures above. Fix the logic in the corresponding files to ensure all tests pass. Update the tests if necessary.`;
  ...
  const handleSend: (customPrompt?: string, isAuto?: boolean, overrideQueue?: string[]) => Promise<void> = useCallback(async (customPrompt?: string, isAuto: boolean = false, overrideQueue?: string[]) => {
    ...
    try {
      ...
      if (allowSupabaseQuestion) {
        setMessages(prev => prev.map(m => m.id === assistantId ? {
          ...m,
          content: "I noticed you need a database for this feature. Please provide your Supabase credentials so I can generate the correct code and SQL schema for you.",
          questions: [{
            id: 'supabase_creds',
      ...
      let currentConfigForAI = { ...projectConfig };
      if (promptText.includes('URL:') && promptText.includes('Key:') && promptText.includes('VITE_SUPABASE_URL')) {
        const urlMatch = promptText.match(/URL:\s*(https:\/\/[^\s]+)/);
        const keyMatch = promptText.match(/Key:\s*([^\s]+)/);
        if (urlMatch && keyMatch) {
          currentConfigForAI.supabase_url = urlMatch[1];
          currentConfigForAI.supabase_key = keyMatch[1];
        }
      }
      ...
      for await (const chunk of stream) {
        ...
        for (const line of lines) {
          try {
            ...
            } else if (update.type === 'plan_progress') {
              setMessages(prev => prev.map(m => m.id === assistantId ? {
                ...m,
                activePlanIndex: update.activePlanIndex,
                completedPlanIndices: update.completedPlanIndices
              } : m));
      ...
      const validQuestions = (res.questions || []).filter((q: any) => {
        ...
        if (q.type === 'supabase_credentials') {
          return false; // Already handled before AI call
        }
      ...
      const finalAssistantMsg: ChatMessage = { 
        ...
        originalFiles: originalFiles,
        validationErrors: messagesRef.current.find(m => m.id === assistantId)?.validationErrors || []
      };
      ...
      // Continue plan if it's a multi-step plan and we are in auto mode, OR if it's the initial request and there are more steps (but NOT in scaffold mode since scaffold implements all steps at once)
      const shouldContinueInitialPlan = !isAuto && nextPlan.length > 1 && res.mode !== 'scaffold';
