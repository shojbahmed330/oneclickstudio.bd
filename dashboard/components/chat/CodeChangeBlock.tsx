...
const CodeChangeBlock: React.FC<CodeChangeBlockProps> = ({ files, originalFiles = {} }) => {
  ...
  const oldCode = useMemo(() => {
    ...
    const code = originalFiles[selectedFile];
    if (typeof code === 'string') return code;
    if (code && typeof code === 'object' && 'content' in code) return String((code as any).content);
    return '';
  }, [selectedFile, originalFiles]);
  ...
  const newCode = useMemo(() => {
    ...
    if (typeof code === 'string') return code;
    if (code && typeof code === 'object' && 'content' in code) return String((code as any).content);
    if (code && typeof code === 'object' && 'code' in code) return String((code as any).code);
    return typeof code !== 'undefined' ? String(code) : '';
  }, [selectedFile, files]);
      ...
      {isOpen && (
        <div className="border-t border-white/5 flex flex-col h-[600px]">
          ...
          <div className="flex items-center justify-between px-4 py-2 bg-black/40 border-b border-white/5">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-all text-[10px] font-bold uppercase tracking-wider border ${rawView ? 'bg-pink-500/20 text-pink-400 border-pink-500/30' : 'bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white border-white/5'}`}
              >
                <FileCode size={12} />
                Raw
              </button>
              <button 
                ...
                disabled={rawView}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-all text-[10px] font-bold uppercase tracking-wider border ${rawView ? 'opacity-50 cursor-not-allowed bg-black/20 text-zinc-600 border-transparent' : 'bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white border-white/5'}`}
              >
                <Split size={12} />
          ...
          <div className="flex-1 overflow-auto custom-scrollbar bg-[#0d0d0f] relative">
            {selectedFile ? (
               rawView ? (
                 <pre className="p-4 text-[12px] font-mono text-zinc-300 whitespace-pre-wrap break-all">
                   {newCode || '/* No content */'}
                 </pre>
               ) : (
                 <ReactDiffViewer
