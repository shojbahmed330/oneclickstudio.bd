...
-- LEARNED ERRORS TABLE (NEW)
CREATE TABLE IF NOT EXISTS public.learned_errors (
  id TEXT PRIMARY KEY,
  pattern TEXT NOT NULL,
  category TEXT NOT NULL CHECK (category IN ('syntax', 'type', 'import', 'runtime', 'config')),
  "rootCause" TEXT NOT NULL,
  "solutionInstruction" TEXT NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc', now()) NOT NULL
);
...
ALTER TABLE public.learned_errors ENABLE ROW LEVEL SECURITY; -- NEW
...
-- LEARNED ERRORS POLICIES (NEW)
CREATE POLICY "anyone_read_learned_errors" ON public.learned_errors FOR SELECT
USING (true);
CREATE POLICY "anyone_insert_learned_errors" ON public.learned_errors FOR INSERT
WITH CHECK (true);
CREATE POLICY "admin_manage_learned_errors" ON public.learned_errors FOR ALL
USING (is_admin_check());
...
IF NOT EXISTS (
SELECT 1 FROM information_schema.columns
WHERE table_name='projects' AND column_name='files'
) THEN
ALTER TABLE public.projects
ADD COLUMN files JSONB DEFAULT '{}'::jsonb;
END IF;
END $$;
-- ১. প্রজেক্ট হিস্ট্রি বা স্ন্যাপশট টেবিল তৈরি
...
-- ২. এই টেবিলের জন্য Row Level Security (RLS) চালু করা
ALTER TABLE public.project_history ENABLE ROW LEVEL SECURITY;
-- ৩. ইউজাররা যেন শুধুমাত্র তাদের নিজেদের প্রজেক্টের হিস্ট্রি দেখতে পারে তার পলিসি
CREATE POLICY "users_history_access" ON public.project_history
FOR ALL
USING (
EXISTS (
